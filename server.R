...
get_selex <- function(aselex, ages){
  Selex_a <- c(1e-20, 1/(1+exp(-log(19)*(ages[-1]-aselex)/((aselex+2)-aselex))))# Selectivity at age
  return(Selex_a)
}
...
get_mature <- function(agemat, ages){
  Mat_a <- c(1e-20, 1/(1 + exp(agemat - ages[-1])))
  return(Mat_a)
}
...
get_SBPR <- function(agemat, Linf, k, t0, lwa, lwb, aselex, M, fish, R0, unfished, text){
  ...
  W_a <- get_weight(ages, Linf, k, t0, lwa, lwb)
  
  Na0 <- Naf <- rep(NA, input$amax)
  Na0[1] <- Naf[1] <- 1
  for (a in 2:input$amax) {
    if (a < input$amax) {
      Na0[a] <- Na0[a - 1] * exp(-M)
...
{
  ages<-c(1:input$amax)
  lengths.out<- get_lengths(input$Linf,input$k,input$t0,ages)
  # plot VBGF
